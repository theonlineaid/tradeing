...
const Portfolio: React.FC<React.PropsWithChildren<Props>> = ({
  layout,
  ...
  const {
    ...
    url: `${_v.API_URL_2}/${_v.USER_PORTFOLIO}/${boInfo?.client_code}/`,
  })

  const {data: profit} = useFetchData({
  ...
  const searchFilterData = (searchValue) => {
    ...
    result = (userPortfolio as any[])?.filter((item: any, k: any) => {
      if (!searchValue.client_code && !searchValue.exchange) return item
      if (item.client_code == searchValue.client_code || item.exchange == searchValue.exchange) {
        return item
      }
    })
      ...
      <div style={{height: '60vh'}}>
        <div id='divToPrint' style={{height: '100%', width: '100%'}} className='ag-theme-balham'>
          <AgGridReact
            ...
            rowData={showSearchData ? searchData : userPortfolio}
